...
def model_space(ang,point):
    ...
    tools = CylinderRegion(axis_length=pm.get_param('TCP')['z'], radius=0.015,
                           base_center=[trans[6][0, 3], trans[6][1, 3], trans[6][2, 3]],
                           axis_direction=[trans[6][0, 2], trans[6][1, 2], trans[6][2, 2]])

    def is_intersecting(cylinder1, cylinder2):
        # 计算两个圆柱体的底面圆心和顶面圆心之间的线段上的10个点
        points1 = [cylinder1.base_center + i / 9 * (
                cylinder1.base_center + cylinder1.axis_direction * cylinder1.axis_length - cylinder1.base_center)
                   for i in range(10)]
        points2 = [cylinder2.base_center + i / 9 * (
                cylinder2.base_center + cylinder2.axis_direction * cylinder2.axis_length - cylinder2.base_center)
                   for i in range(10)]
        # 计算这20个点之间的距离(400)
        ...
        #distances = [np.linalg.norm(p1 - p2) for p1, p2 in zip(points1, points2)] + \
                    #[np.linalg.norm(p1 - p2) for p1, p2 in zip(points1, reversed(points2))]#末端工具圆柱底面与第六连杆相接触
        # 如果有一个距离小于两个半径之和，那么这两个圆柱体(胶囊体)相交
        if any(d < cylinder1.radius + cylinder2.radius for d in distances):
            return True
        else:
            return False
    ...
    def check_interference(point):
        ...
        # 检查工具与其他圆柱体之间是否相交
        for i in range(len(cylinders) - 3):  # 确保不包括"tools"本身以及第五、六连杆，
            if is_intersecting(cylinders[i], cylinders[-1]):  # 始终与"tools"（cylinders列表的最后一个元素）进行比较
                return "NO"  # 干涉
            break
        else:
