...
fig = plt.figure(figsize=(6, 4)) # 宽度和高度的尺寸，单位是英寸
...
async def main():
    ...
    with open('simple_euler.csv', 'r', encoding='utf-8') as file:
        ...
        for i, row in enumerate(reader, 1):  # 添加了enumerate函数，用于计数
            ...
            current_j6 = current_joints['actual_joint_pose'][5]  # 获取当前j6旋转角度
            ...
            if rollback_enabled and abs(current_j6 - initial_j6) > 2 * math.pi:
                ...
                await lebai.speedl(0.1, {'x': 0, 'y': 0, 'z': -0.05, 'rz': 0, 'ry': 0, 'rx': 0}, 0, {'x': float(row[0]), 'y': float(row[1]), 'z': float(row[2]), 'rz': float(row[3]), 'ry': float(row[4]), 'rx': float(row[5])})

                current_joints = await lebai.get_kin_data()  # 获取当前关节位置

                # 使得末端关节回退回初始旋转角度
                await lebai.movej([current_joints['actual_joint_pose'][0],current_joints['actual_joint_pose'][1],current_joints['actual_joint_pose'][2]
